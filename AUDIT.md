...
| **B5** | Refresh and staleness | `meta_refresh` records `fetched_utc`, the window, the station list and `m_version` on every refresh; `study.py` copies them into `manifest.json`, and marks a study `incomplete` rather than guessing if the sheet is absent. |
